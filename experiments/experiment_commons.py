...
from ware_ops_sim.data_loaders import IWSPELoader, IOPVRPLoader

from ware_ops_sim.sim.events.events import OrderArrival, RoutingDone, OrderPriorityChange, PickerArrival, \
    PickerTourQuery, OrderSelectionDone, TourEnd, PickerIdle
...
from ware_ops_algos.algorithms import PlanningState, CombinedRoutingSolution, AssignmentSolution, RoundRobinAssigner, \
    PickList, AlgorithmSolution, Algorithm, I, O, GreedyItemAssignment, PickListRouting, WarehouseOrder, Job
...
def build_trigger_map(cfg: DictConfig) -> dict:
    ...
    return {
        ...
        for event_name, policy_key in cfg.simulation.triggers.items()
    }
...
def build_req_policy(cfg: DictConfig) -> dict:
    ...
    return {
        ...
        for policy_key, condition_cfg in cfg.simulation.conditions.items()
    }
...
def build_state_transformers(cfg: DictConfig) -> dict:
    return {
        ...
        for problem_key, st_cfg in cfg.simulation.state_transformer.items()
    }

def build_data_loader(cfg: DictConfig) -> DataLoader:
    data_loader_cls = cfg.data_cards.source.data_loader
...
def make_loader(cfg, instances_dir, cache_dir):
    loader_type = cfg.data_cards.loader
    if loader_type == "IWSPELoader":
        return IWSPELoader(
            instances_dir=instances_dir,
            cache_dir=cache_dir,
            cfg=cfg
        )
    elif loader_type == "IOPVRPLoader":
        return IOPVRPLoader(
            instances_dir=instances_dir,
            cache_dir=cache_dir,
        )

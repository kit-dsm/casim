...
from ware_ops_algos.algorithms.waiting import decide_henn
...
def _decide_henn_call(candidate, snapshot, **kwargs):
    picker = snapshot.resources.resources[0]
    orders = []
    if hasattr(snapshot, "orders"):
        orders = list(getattr(snapshot.orders, "orders", []) or [])
    return decide_henn(
        candidate,
        picker=picker,

...
class TourStart(BaseTourEvent):
    def handle(self, state: 'SimulationState') -> list['Event']:
        ...
        logger.info(f"tour contains {len(tour.pick_positions)} items")
        logger.debug("Pick Positions")
...
class TravelEvent(BaseTourEvent):
    ...
    def handle(self, state: SimulationState) -> list[Event]:
        ...
        # print("Congestion", congestion_rate)
        congestion_factor = 1
        # if tour.tour_id == 2:
        #     print(f"Current position picker {res.id}: {origin}")
        if n_cobots_in_aisle > 0:
            congestion_factor = 1.0 + congestion_rate * (n_cobots_in_aisle - 1)
        travel_time = travel_time * congestion_factor
        # print(f"N cobots in aisle: {n_cobots_in_aisle - 1}, Congestion {congestion_factor}")
...
class NodeArrival(BaseTourEvent):
    ...
    def handle(self, state: SimulationState) -> list[Event]:
        ...
        if tour.at_end():
            # if tour.tour_id == 2:
                # print("finished at ", here)
            return [TourEnd(self.time, tour.tour_id)]

        # If next planned pick is exactly here → start pick
        # print("picks left", tour.picks_left)
        # if tour.picks_left and tour.picks_left[0] == here:
        #tour.picks_left and here in
        # if here in tour.pick_positions and tour.visits_left[here] > 0:
        # if here in tour.pick_sequence: # and here in tour.pick_nodes:
        # if tour.tour_id == 2:
        #     print("Here", here)
        if here.node_type == NodeType.PICK:
            finish_at = self.time + res.time_per_pick
            # state.tracker.update_on_pick_start({
            #     'type': 'pick_start',
        ...
        # assert not tour.at_end(), f"At end of route but not at depot: {here}"
        return [TravelEvent(self.time, tour.tour_id)]
...
class PickComplete(BaseTourEvent):
    ...
    def handle(self, state: SimulationState) -> list[Event]:
        ...
        # Validate we're still at the right node and that the next pick matches
        # assert tour.picks_left, f"No picks left but PickComplete issued on tour {tour.tour_id}"
        # assert tour.picks_left[0] == here, f"PickComplete at {here} but next pick is {tour.picks_left[0]}"

        # Pop the next pick and mark fulfilled positions
        # popped = state.tour_manager.pop_next_pick_if_here(tour.tour_id, here)
        # try:
            # assert popped, "Failed to pop next pick at current location"
        # except:
...
class TourEnd(BaseTourEvent):
    def handle(self, state: SimulationState) -> list[Event]:
        ...
        logger.info(f"resource {res.id} finished picking orders {tour.order_numbers} at t={self.time}")

        logger.info(f"Tour {tour.tour_id} completed at depot. "
                     f"Planned end: {tour.end_time_planned}, actual end: {self.time}")


        # print(f"resource {res.id}: Tour {tour.tour_id} completed at depot. "

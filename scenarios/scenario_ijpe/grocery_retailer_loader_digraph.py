...
class WarehousePickingLoaderDigraph(DataLoader):
    ...
    def _build_layout(self, layout_raw: pd.DataFrame) -> tuple[LayoutData, list[StorageLocation]]:
        ...
        G.add_edge(start_location, closest_node_to_start, weight=0, edge_type="depot")
        G.add_edge(end_location, closest_node_to_start, weight=0, edge_type="depot")
    ...
    # ------------------------------------------------------------------
    # Optional historic replay jobs
    # ------------------------------------------------------------------

    def _build_jobs_replay(self, orders_df: pd.DataFrame):
        if orders_df[COL_START_SEC].isna().all():
            orders_df[COL_START_SEC] = (
                pd.to_timedelta(orders_df["BEGINN_ZEIT"].astype(str))
                .dt.total_seconds()
            )
            orders_df[COL_END_SEC] = (
                pd.to_timedelta(orders_df["ENDE_ZEIT"].astype(str))
                .dt.total_seconds()
            )

        releases = (
            orders_df
            .groupby([COL_ORDER_ID, COL_DATE])[COL_START_SEC]
            .min()
            .groupby(COL_ORDER_ID)
            .min()
            .to_dict()
        )
        ...
        deadlines = (
            orders_df
            .groupby([COL_ORDER_ID, COL_DATE])[COL_END_SEC]
            .max()
            .groupby(COL_ORDER_ID)
            .max()
            .to_dict()

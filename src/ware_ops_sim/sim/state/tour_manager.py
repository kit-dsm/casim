...
class TourManager:
    ...
    def create_tour(self, route_plan: Route) -> int:
        ...
        new_tour = TourPlanningState(
            tour_id=tour_id,
            route_nodes=list(route_plan.route),
            pick_sequence=list(route_plan.item_sequence),
            picks_left=deque(route_plan.item_sequence),
            # pick_positions=deepcopy(route_plan.pick_list.pick_positions),
            pick_positions=deepcopy(route_plan.pick_list.pick_positions),
            ...
            pick_nodes=[pp.pick_node for pp in route_plan.pick_list.pick_positions],
            visits_left=Counter(route_plan.item_sequence),
            annotated_route=route_plan.annotated_route
        )

        pick_nodes = [n for n in route_plan.annotated_route if n.node_type == NodeType.PICK]
        route_nodes = [n for n in route_plan.annotated_route if n.node_type == NodeType.ROUTE]
        # print("Pick_nodes", pick_nodes)
        # print(len(new_tour.pick_sequence))
        # print(len(pick_nodes))
        # print(pick_nodes)
        # print(new_tour.pick_sequence)
        # assert len(pick_nodes) == len(new_tour.pick_sequence), print(pick_nodes)

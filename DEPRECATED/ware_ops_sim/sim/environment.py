...
from ware_ops_algos.algorithms.algorithm import PlanningState, RoutingSolution, AssignmentSolution, SchedulingSolution, \
    CombinedRoutingSolution, PickList, OrderSelectionSolution, AlgorithmSolution, NodeType, BatchingSolution, \
...
from ware_ops_sim.sim.state.state_transformer import StateSnapshot
...
from ware_ops_sim.sim.events.events import OrderArrival, Event, TourStart, SequencingDone, RoutingDone, \
    PickListDone, BaseTourEvent, PickerArrival, OrderSelectionDone, PickListSelectionDone, FlushRemainingOrders
...
class WarehouseSimulation:
    ...
    """
    _SHARED_FIELDS = {'state_transformers', 'control', 'data_loader',
                      'cache_path', 'domain_cache_path'}
    ...
    def run(self):  #  step_no_action()
        ...
        while self.events:
            ...
            if event.__class__ in self.control.triggers.keys():
                ...
                if isinstance(event, BaseTourEvent):
                    ...
                    self.state.current_picker_id = event.picker_id
                elif isinstance(event, PickListSelectionDone):
                    self.state.current_picker_id = event.picker_id
                else:
                ...
                dynamic_info = state_transformer.transform_state(self.state,
                                                                 problem)
                if isinstance(event, OrderArrival) and hasattr(state_transformer, "selected_picker"):
                    self.state.current_picker_id = state_transformer.selected_picker # TODO VERY BAD DESIGN CHOICE!
                ...
                if condition.get_decision(dynamic_info):  # TODO make sure we do not have to check this twice
                    ...
                    logger.info("Not condition", len(dynamic_info.resources.resources))
        ...
        logger.debug("buffer", len(self.state.order_manager.get_order_buffer()))
    ...
    def __on_trigger(self, event: Event, dynamic_info: SimWarehouseDomain) -> AlgorithmSolution:
        ...
        if condition.get_decision(dynamic_info):
            logger.info(f"{condition} triggered at {self.state.current_time}")
            runner = self.control.get_execution(problem)
    ...
    def solution_to_events(self, solution: AlgorithmSolution):
        ...
        if isinstance(solution, OrderSelectionSolution):
        ...
        elif isinstance(solution, PickListSelectionSolution):
            events_to_return = self._pick_list_selection_to_events(solution)

        elif isinstance(solution, CombinedRoutingSolution):
            logger.info("Resource before Routes Creation")
            ...
            events_to_return = self._batches_to_events(solution)
    ...
    # TODO couple these functions directly with the config / runner classes to
    #  make explicit what needs to be returned back to the sim?
    ...
    def _pick_list_selection_to_events(self, pl_selection_sol: PickListSelectionSolution):
        events_to_return: list[Event] = []
        pl_selected = pl_selection_sol.selected_pick_lists
        for o in pl_selected:
            events_to_return.append(PickListSelectionDone(time=self.state.current_time,
                                                          selected_order=o,
                                                          picker_id=self.state.current_picker_id))
        return events_to_return
    ...
    def _schedules_to_events(self, sequencing_sol: SchedulingSolution):
        ...
        """
        events_to_return: list[Event] = []
        sequences = [sequencing_sol.jobs[0]] # TODO BAD!!
        assignments = sorted(sequences, key=lambda a: (a.picker_id, a.start_time))
        # print(assignments)
        for a in assignments:
            logger.debug("picker in sequence: ", a.picker_id)
            logger.debug(f"Tour: {a.batch_idx}, start: {a.start_time}, end: {a.end_time}, distance: {a.route.distance}")
            # print(f"picker speed: {self.state.resource_manager.get_resource(a.picker_id).speed}")
            pick_nodes = [n for n in a.route.annotated_route if n.node_type == NodeType.PICK]
            logger.debug("pick nodes", len(pick_nodes))
    ...
    def _batches_to_events(self, batching_solution: BatchingSolution):
        events_to_return = []
        batches = batching_solution.batches
        pls = batching_solution.pick_lists
        for pl in pls:
            events_to_return.append(PickListDone(self.state.current_time, pl))
        return events_to_return
    ...
    def __deepcopy__(self, memo):
        ...
        for k, v in self.__dict__.items():
            if k in self._SHARED_FIELDS:
                ...
                result.events = self._copy_events(v, memo)
            else:
                setattr(result, k, copy.deepcopy(v, memo))
        return result

    def _copy_events(self, heap, memo):
        from ware_ops_sim.sim.events.events import OrderArrival
        new_heap = []
        for event in heap:
            if isinstance(event, OrderArrival):
                new_heap.append(event)  # shared ref
            else:
                new_heap.append(copy.deepcopy(event, memo))
        return new_heap
